...
public class as97164Server extends JFrame implements Runnable{
	//사용자가 접속을 해올 때마다(입장) 새로 객체를 생성해야 함. - 왜냐하면 서로 다른 사람이니까. 그 사람의 소켓, 그사람만의ois, 그사람만의oos.
	as97164ServerThread 	as97164st 		= null;
	//Vector로 선언하지 않는 이유는 다형성을 말하기 위함.
	//내랑레 강감찬 스레드 관리, 이순신 스레드 관리
	List<as97164ServerThread> 	globalList 	= null;
...
//서버소켓과 클라이언트측 소켓을 연결하기
	...
	public void run() {
		...
		try {
			...
			while(!isStop) {
				...
				as97164st = new as97164ServerThread(this);
				as97164st.start();
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static void main(String args[]) {
		as97164Server as97164s = new as97164Server();
		as97164s.initDisplay();
		Thread th = new Thread(as97164s);
		th.start();
